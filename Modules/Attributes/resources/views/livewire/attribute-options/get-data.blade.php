            ...
            @forelse ($data as $value)
                <tr>
                    ...
                    <td class="d-flex flex-wrap" >
                        @foreach ($value->value ?: [] as $key => $translated)
                            <p class="mx-2"><x-core::partials.badge :label="strtoupper($key)" /> : {{ $translated }}</p>
                        @endforeach
                    </td>
                    <td><b><x-core::partials.badge :label="$value->attribute->value[app()->getLocale()]" /></b></td>
                    <td><b><x-core::partials.date-format :date="$value->created_at" /></b></td>

                    <x-core::tables.partials.action-button
                        :actions="[
                            ['label' => __('Details'), 'event' => 'show_attribute_option', 'id' => $value->id, 'icon' => 'bx bx-info-circle'],
                            ['label' => __('Edit'), 'event' => 'edit_attribute_option', 'id' => $value->id, 'icon' => 'bx bx-edit'],
                            ['divider' => true],
                            ['label' => __('Delete'), 'event' => 'delete_attribute_option', 'id' => $value->id, 'icon' => 'bx bx-trash', 'class' => 'text-danger', 'confirm' => true],
                        ]"
                    />
